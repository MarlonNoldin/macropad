...
class KMKKeyboard:
    ...
    num_pixels = 10
    brightness = 0.5
    pixels = neopixel.NeoPixel(board.D10, num_pixels, brightness = brightness)
    pre_layer = 1
    delay = 0.1
    ...
    def go(self, hid_type=HIDModes.USB, secondary_hid_type=None, **kwargs,) -> None:
        ...
        self.set_neopixel_color_by_layer(0)
        try:
            while True:
                self._main_loop()
        finally:
    ...
    #Modified Library Code for switching layer through Windows Skript  
    def nachricht_to_layer(self,data):
        parts = data.split(':')
        if len(parts) == 2 and parts[0] == 'Ws':
            try:
                ...
                return layer
                 ...
                 return None  # Falls die Zahl nicht in ein int umgewandelt werden kann
        else:
             return None
    
    def set_neopixel_color_by_layer(self, layerInt):
        # Define colors for each layer
        layer_colors = {
            0: (0, 0, 255),    # Windows Blue
            1: (255, 255, 0),  # Illustrator Yellow
            2: (128, 0, 128),    # Photoshop Lila
            3: (255,0,0),# Visual Studio Rot 
            4: (255, 127, 0), 
            5: (255, 0,0)  # Fusion Orange   
        }
        color = layer_colors.get(layerInt, (0, 0, 0))  # Default to off if layer not defined
        self.pixels.fill(color)
        self.pixels.show()
         
    def switch_to_layer(self,layerInt) -> None:
       if(self.pre_layer != layerInt):
            if layerInt == 0:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(0))
            elif layerInt == 1:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(1))
            elif layerInt == 2:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(2))
            elif layerInt == 3:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(3))
            elif layerInt == 4:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(4))
            elif layerInt == 4:
                self.set_neopixel_color_by_layer(layerInt)
                self.tap_key(KC.TO(5))
            
       else:
           self.pre_layer = layerInt        
    ...
    def _main_loop(self) -> None:
        ...
        if usb_cdc.data.in_waiting > 0:
            data = usb_cdc.data.read(usb_cdc.data.in_waiting).decode('utf-8').rstrip()
            
            currentLayer = self.nachricht_to_layer(data)            
            response = self.switch_to_layer(currentLayer)
            
            combined_response = "Empfangen: " + str(currentLayer) + "\n"
            
            usb_cdc.data.write(combined_response.encode('utf-8'))
